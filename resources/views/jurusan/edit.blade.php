<x-app-layout>
    ...
    <div class="max-w-4xl mx-auto py-10 sm:px-6 lg:px-8">
        <form action="{{ route('jurusan.update', $jurusan->id) }}" method="POST" class="space-y-4 bg-white dark:bg-gray-800 p-6 rounded shadow">
            @csrf
            @method('PUT')
            ...
            <div>
                ...
                <select name="guru_id" id="guru_id" class="w-full bg-white dark:bg-gray-700 text-gray-900 dark:text-gray-100 border rounded px-3 py-2">
                    ...
                    @foreach ($gurus as $guru)
                        <option value="{{ $guru->id }}" {{ $jurusan->guru_id == $guru->id ? 'selected' : '' }}>{{ $guru->nama }} - {{ $guru->mapel }}</option>
                    @endforeach
                </select>
            </div>
